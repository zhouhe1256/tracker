...
//        });
    ...
    private class PathOverlay extends Overlay {
        ...
        private double avgX=0.000004;
        private double avgY=0.00288;
        @Override
...
//                    canvas.drawPath(path, paint);
                    ...
                    for(int i=1;i<locations.size()-1;i++){
                        //附近的2个点 同时有效 才可以
                            if(max<locations.get(i).getSpeed()){ //求最大额 并复制
                                max=locations.get(i).getSpeed(); //最大值
                            }
                            if(min>locations.get(i).getSpeed()){ //求最小额 并复制
                    ...
                    for(int i=0;i<locations.size()-1;i++){
                        Point current = projection.toPixels(getRealGeoPointFromLocation(locations.get(i)), null);
                        Point next = projection.toPixels(getRealGeoPointFromLocation(locations.get(i+1)), null);
                            //红色
                            ...
                            float x1=next.x;
                            float y1=next.y;
                            paint.setColor(Color.rgb(red, green, 00));
